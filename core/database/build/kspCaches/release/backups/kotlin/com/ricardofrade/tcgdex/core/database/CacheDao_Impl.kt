...
public class CacheDao_Impl(
  ...
  init {
    ...
    this.__insertAdapterOfCacheEntity = object : EntityInsertAdapter<CacheEntity>() {
      protected override fun createQuery(): String = "INSERT OR REPLACE INTO `generic_cache` (`key`,`payload`,`timestamp`) VALUES (?,?,?)"
  ...
  public override fun getCache(key: String): CacheEntity? {
    val _sql: String = "SELECT * FROM generic_cache WHERE `key` = ?"
  ...
  public override fun clearCache() {
    val _sql: String = "DELETE FROM generic_cache"
  ...
  public override fun deleteCache(key: String) {
    val _sql: String = "DELETE FROM generic_cache WHERE `key` = ?"
    return performBlocking(__db, false, true) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
